...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        temp_input_ids: torch.LongTensor = None,
        select_layer: Optional[List[int]] = None,
        ...
        vs_pos = (temp_input_ids == vision_start_id).nonzero(as_tuple=False)
        ve_pos = (temp_input_ids == vision_end_id).nonzero(as_tuple=False)
        for i, decoder_layer in enumerate(self.layers):
            ...
            if select_layer is not None and i in select_layer:
                if delim_scaling and len(vs_pos) > 1:

                    for pos in vs_pos[:]:
                        hidden_states[pos[0], pos[1]] *= scale

                if delim_scaling and len(ve_pos) > 1:
                    for pos in ve_pos[:]:
                        hidden_states[pos[0], pos[1]] *= scale
            if use_cache:
